...
struct option
{
    option(std::string _short, std::string _long, std::string _help, std::string _type, std::string _value) : 
        short_str(std::move(_short)),
        long_str(std::move(_long)),
        help(std::move(_help)),
        type(std::move(_type)),
        value(std::move(_value)) {}

    void print_help() const {
        if(this->short_str == "") std::cout << "     ";
        else std::cout << std::setw(4) << this->short_str << ",";
        if(this->long_str == "") std::cout << "          ";
        else std::cout << std::setw(10) << this->long_str;
        std::cout << "          " << this->help;
        if(this->type != "bool")
        {
            std::cout << " (" << this->type << ")";
...
struct argument{
    argument(std::string _name, std::string _help) :
        name(std::move(_name)), help(std::move(_help)) {}
    
    bool has() const {return this->value != "";}
    void print_help() const {
        std::cout << std::setw(15) << this->name << "          "
                  << this->help << std::endl;
    }
...
public:
    ...
    argparser &add_option(const std::string &_short, const std::string &_long, const std::string &_help, T _value)
    {
        ...
        this->options.push_back(
            option(_short, _long, _help, type_string<T>::value, value_str)
        );
        return *this;
    }
    ...
    argparser &add_option(const std::string &_short, const std::string &_long, const std::string &_help)
    {
        ...
        this->options.push_back(
            option(_short, _long, _help, "bool", "")
        );
        return *this;
    }
    ...
    argparser &parse(int argc, char const *argv[])
    {
        ...
        {
            std::cerr << "argument number missmatching, give " << args.size() << ", but need " << this->arguments.size() << std::endl;
            std::exit(-1);
        }
        else
        {
            ...
            std::for_each(arguments.begin(), arguments.end(),
                [&pos](argument &arg){arg.value = *pos++;}
            );
        }
        return *this;
    }
...
private:
    ...
    void print_help() const {
        ...
        std::cout << "options:\n";
        std::for_each(this->options.cbegin(), this->options.cend(),
            [](const option &opt){opt.print_help();}
        );
    }
