...
struct IniSection {
    ...
    bool get_bool(const string &key) {
        ...
        std::cerr << "Section " << this->name << ", key " << key
                  << ", invalid bool value: " << pos->value << std::endl;
        std::exit(-1);
    }
...
private:
    ...
    vector<IniSection>::iterator check_section(const string &name){
        auto pos = std::find_if(sections.begin(), sections.end(), [&name](const IniSection &sec){return sec.name == name;});
        if(pos == sections.end())
        {
            if(name == "") std::cerr << "on default section" << std::endl;
            else std::cerr << "section not found: " << name << std::endl;
            std::exit(-1);
        }
        return pos;
    }
